...
A Python script to extract DNS queries from `.pcap` or `.pcapng` files using PyShark. It captures unique DNS queries, shows the source IP, queried domain, and detects the DNS server IP.
...
- For each packet, it extracts:
  - **Source IP** — the host making the query
  - **Queried domain name** — the domain being looked up
...
| **DNS Response** | Reply from the DNS server containing the IP address |
| **`dns.flags.response == 0`** | Wireshark filter that isolates query packets only |
